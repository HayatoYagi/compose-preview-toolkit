...
class NavGraphSiteTest {
    ...
    fun `buildGallerySiteHtml renders every node's data and a placeholder-capable modal for unmatched thumbnails`() {
        val nodes = listOf(
            GalleryNode(qualifiedName = "com.example.HomeRoute", simpleName = "HomeRoute", packageName = "com.example", thumbnails = emptyList()),
            GalleryNode(
                qualifiedName = "com.example.featurea.FeatureARoute",
                simpleName = "FeatureARoute",
                packageName = "com.example.featurea",
    ...
    fun `buildGallerySiteHtml escapes a double quote in a route name so it can't break out of the embedded JS string literal`() {
        val nodes = listOf(
            GalleryNode(qualifiedName = "com.example.Weird\"Route", simpleName = "Weird\"Route", packageName = "com.example", thumbnails = emptyList()),
        )
    ...
    @Test
    fun `buildGallerySiteHtml links back to the toolkit's own repo in a footer`() {
        val html = buildGallerySiteHtml(emptyList(), mermaidGraph = "graph TD;\n")

        assertTrue(html.contains("Generated by"))
        assertTrue(html.contains("https://github.com/HayatoYagi/compose-preview-toolkit"))
    }
    ...
    @Test
    fun `buildGallerySiteHtml omits the build-info footer line when buildInfo is null`() {
        val html = buildGallerySiteHtml(emptyList(), mermaidGraph = "graph TD;\n")

        assertFalse(html.contains("Built from commit"))
    }

    @Test
    fun `buildGallerySiteHtml renders a linked short SHA, branch, and PR number when buildInfo has a repository`() {
        ...
        assertTrue(html.contains("Built from commit"))
        assertTrue(
            html.contains(
                "<a href=\"https://github.com/HayatoYagi/compose-preview-toolkit/commit/abc1234567890\" " +
                    "target=\"_blank\" rel=\"noopener\">abc1234</a>",
            ),
        )
        assertTrue(html.contains("on branch <code>feature/foo</code>"))
        assertTrue(
            html.contains(
                "(<a href=\"https://github.com/HayatoYagi/compose-preview-toolkit/pull/69\" " +
                    "target=\"_blank\" rel=\"noopener\">PR #69</a>)",
            ),
        )
    }

    @Test
    fun `buildGallerySiteHtml renders the plain short SHA with no link when buildInfo has no repository`() {
        val buildInfo = SiteBuildInfo(commitSha = "abc1234567890")

        val html = buildGallerySiteHtml(emptyList(), mermaidGraph = "graph TD;\n", buildInfo = buildInfo)

        assertTrue(html.contains("Built from commit abc1234."))
        // The footer's own "Generated by" link is always present; only a commit/pull link should be absent.
        assertFalse(html.contains("/commit/"))
        assertFalse(html.contains("/pull/"))
    }

    @Test
    fun `buildGallerySiteHtml build-info footer omits branch and PR segments when they're absent`() {
        val buildInfo = SiteBuildInfo(commitSha = "abc1234567890", githubRepository = "HayatoYagi/compose-preview-toolkit")

        val html = buildGallerySiteHtml(emptyList(), mermaidGraph = "graph TD;\n", buildInfo = buildInfo)

        assertFalse(html.contains("on branch"))
        assertFalse(html.contains("PR #"))
    }
    ...
    fun `buildMermaidGraph assigns positional ids and keeps qualified names only in labels`() {
        val nodes = listOf(
            ...
            GalleryNode("com.example.featurea.FeatureARoute", "FeatureARoute", "com.example.featurea", emptyList()),
        )
    ...
    fun `buildMermaidGraph renders a node with no outgoing or incoming edges`() {
        val nodes = listOf(GalleryNode("com.example.TerminalRoute", "TerminalRoute", "com.example", emptyList()))
    ...
    fun `buildMermaidGraph renders a real cycle (A to B and B to A) as two edge lines, not a crash or dedup`() {
        val nodes = listOf(
            GalleryNode("com.example.featurea.FeatureARoute", "FeatureARoute", "com.example.featurea", emptyList()),
            GalleryNode("com.example.featureb.FeatureBRoute", "FeatureBRoute", "com.example.featureb", emptyList()),
        )
        val edges = listOf(
    ...
    fun `buildMermaidGraph drops an edge referencing a route absent from nodes instead of emitting a dangling id`() {
        val nodes = listOf(GalleryNode("com.example.HomeRoute", "HomeRoute", "com.example", emptyList()))
    ...
    fun `buildMermaidGraph escapes a quote in a label without breaking mermaid's quoted-string syntax`() {
        val nodes = listOf(GalleryNode("com.example.Weird\"Route", "Weird\"Route", "com.example", emptyList()))
    ...
    @Test
    fun `buildMermaidGraph labels a nested route with its outer-to-inner chain, not just its own leaf simple name`() {
        // Mirrors a route nested inside a sealed hierarchy, e.g. TodoRoute.Detail (see
        // NavNode's own kdoc for this same example) — simpleName alone ("Detail") loses the
        // disambiguating parent and reads as a bare, generic word on the rendered graph.
        val nodes = listOf(
            GalleryNode(
                qualifiedName = "com.example.TodoRoute.Detail",
                simpleName = "Detail",
                packageName = "com.example",
                thumbnails = emptyList(),
            ),
            GalleryNode(
                qualifiedName = "com.example.TodoRoute.Summary",
                simpleName = "Summary",
                packageName = "com.example",
                thumbnails = emptyList(),
            ),
        ...
        assertTrue(graph.contains("n0[\"TodoRoute.Detail\"];"))
        assertTrue(graph.contains("n1[\"TodoRoute.Summary\"];"))
        assertFalse(graph.contains("n0[\"Detail\"];"))
        assertFalse(graph.contains("n1[\"Summary\"];"))
    }
}
