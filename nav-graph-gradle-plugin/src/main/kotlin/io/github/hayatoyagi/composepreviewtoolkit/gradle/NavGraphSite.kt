...
data class GalleryNode(
    ...
    /**
     * [qualifiedName] with the leading [packageName] stripped — the outer-to-inner simple-name
     * chain a reader actually needs to tell routes apart (e.g. `TodoRoute.Detail`),
     * without the package noise [qualifiedName] also carries. Unlike [simpleName] (just this
     * route's own leaf name, e.g. `Detail` — ambiguous for a route nested inside a sealed hierarchy,
     * since a sibling route elsewhere could share it), this is what [buildMermaidGraph] labels
     * nodes with.
     */
 ...
 * punctuation that a fully-qualified Kotlin name routinely has, e.g. `com.example.TodoRoute.Detail`
    ...
    return buildString {
        ...
        nodes.forEachIndexed { index, node ->
            ...
            if (representative != null) {
                append(
                    "$nodeId@{ img: \"${representative.dataUri}\", label: \"${node.nestedName.mermaidLabel()}\", " +
                        "pos: \"b\", h: $THUMBNAIL_NODE_HEIGHT_PX, constraint: \"on\" }",
                ).append(";\n")
            } else {
                append("$nodeId[\"${node.nestedName.mermaidLabel()}\"]").append(";\n")
            }
...
fun buildGallerySiteHtml(
    ...
    return """
        ...
        .cpt-footer { text-align: center; color: #a1a1a6; font-size: 12px; margin-top: 40px; }
        .cpt-footer a { color: inherit; }
        </style>
        </head>
        <body>
