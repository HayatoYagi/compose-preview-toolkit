 ...
 * route declaration) — see `EntryRegistrations.kt`'s `locateCallSite` for how this is computed.
 * [filePath] is always a short, human-readable path, never an absolute machine-specific one:
 * [filePathIsRepoRelative] tells callers whether it's relative to the git repository root (and
 * therefore safe to build a GitHub blob URL from, see `nav-graph-gradle-plugin`'s
 * `buildSourceLink`) or is only a best-effort fallback (relative to some other base directory, or
 * — for a synthetic/in-memory [org.jetbrains.kotlin.psi.KtFile] with no real file backing it,
 * e.g. in a unit test — just the raw name PSI was given).
 */
data class NavNode(
